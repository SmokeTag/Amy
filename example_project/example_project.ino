...
// =====================================================================================
// --- Function Prototypes ---
void read_bts();            // Read if any of the buttons got pressed
bool read_bt_enter();       // Read if enter button got pressed return 1(True) or 0(False)
bool read_bt_menu();        // Read in menu button got pressed return 1(True) or 0(False)
bool read_bt_down();        // Read in down button got pressed return 1(True) or 0(False)
bool read_bt_up();          // Read in up button got pressed return 1(True) or 0(False)

void menu_select();         // Open up the submenu for the selected option
float read_temperature();   // Reads the temperature from the heat sensor and return the temperature in ºC
...
// =====================================================================================
// --- Inicial config ---
LiquidCrystal lcd(rs, en, db0, db1, db2, db3, db4, db5, db6, db7); // Inicializes the liquid crystal libriry
void setup() {
  lcd.begin(16, 2);                  // starts the liquid crysta display (LCD)
  pinMode(menu_bt, INPUT_PULLUP);    // set pin 19 (menu_bt) as input and activates the pull up
  pinMode(enter_bt, INPUT_PULLUP);
  pinMode(up_bt, INPUT_PULLUP);
  pinMode(down_bt, INPUT_PULLUP);
  pinMode(led, OUTPUT);              // set the pin 2 (led) as output
}  //end setup
...
bool read_bt_up() {
  ...
  unsigned long current_time = millis();
  //UP BUTTON
  if (!digitalRead(up_bt) && (current_time - up_bounce > 3) && !sub_menu) {  //if(button is pressed and it's been more than 3ms since last "press")
...
bool read_bt_down() {
  ...
  unsigned long current_time = millis();
  //DOWN BUTTON
  if (!digitalRead(down_bt) && (current_time - down_bounce > 3) && !sub_menu) {  //if(button is pressed and it's been more than 3ms since last "press")
